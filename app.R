...
state_or_us_ts_plot <- function(data, state_name) {
  ...
  plot_ly(
    ...
    hovertemplate = paste(
      "Week: %{x}<br>",
      "Dose per capita: %{y:.3f}<extra></extra>"
    )
  ) %>%
    layout(
      title = paste("Cumulative Vaccine Allocation per Capita -", state_name),
      xaxis = list(title = "Week"),
      yaxis = list(title = "Dose per Capita")
    )
}

ranking_table <- function(data, selected_date, rank_type) {
  
  ranking_data <- data %>%
    filter(Week.of.Allocations == selected_date) %>%
    select(Jurisdiction, pop, DosePerPop) %>%
    arrange(desc(DosePerPop))
  ...
  ranking_data %>%
    mutate(
      ...
      `Dose Per Population` = round(DosePerPop, 3)
    ) %>%
    select(
      State = Jurisdiction,
      Population,
...
ui <- fluidPage(
  ...
  titlePanel("COVID-19 Vaccine Allocation per Capita"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      br(),
      h4("State Ranking by Dose Per Capita"),
...
server <- function(input, output, session) {
  ...
  output$ranking_table <- renderTable({
    ranking_table(all_data, input$plot_date, input$rank_type)
  })
}
